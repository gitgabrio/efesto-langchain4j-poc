 ...
 * under the License.
 */
package org.kie.lc4j.efesto.compiler.utils;
 ...
 * <b>code-generation</b>
 */
// Q'n'd carbon-copy of org.kie.pmml.compiler.commons.utils.JavaParserUtils
public class JavaParserUtils {
    ...
    public static CompilationUnit getFromFileName(String fileName) {
        try {
            ...
            throw new KieLC4JException(String.format("Failed to parse %s due to %s", fileName,
                                                             e.getMessage()), e);
        } catch (Exception e) {
            throw new KieLC4JException(String.format("Failed to read %s due to %s", fileName, e.getMessage()), e);
        }
    }
    ...
    public static CompilationUnit getFromSource(String source) {
        try {
            ...
            throw new KieLC4JException(String.format("Failed to parse\r\n%s\r\ndue to %s", source, e.getMessage()), e);
        }
    }
    ...
    public static CompilationUnit getKiePMMLModelCompilationUnit(final String className,
        ...
        ClassOrInterfaceDeclaration modelTemplate = toReturn.getClassByName(modelClassName)
                .orElseThrow(() -> new KieLC4JException(MAIN_CLASS_NOT_FOUND + ": " + modelClassName));
     ...
     * It throws <code>KieLC4JException</code> if the package name is missing
     *
     * @param cu
     * @return
     */
    public static String getFullClassName(final CompilationUnit cu) {
        String packageName = cu.getPackageDeclaration()
                .orElseThrow(() -> new KieLC4JException("Missing package declaration for " + cu.toString()))
                .getName().asString();
